...
export default function FileUpload({
  ...
  const acceptedTypes = accept === 'all' 
  ...
  const simulateUpload = async () => {
    ...
    for (const fileObj of files) {
      ...
      try {
        ...
        const response = await api.post(uploadEndpoint, {
          file: fileObj.file,
        }, {
  ...
  const progressPercent = Object.values(uploadProgress).length > 0
    ...
    <div className={className}>
      ...
      >
        ...
        <div className="flex flex-col items-center">
          ...
          `}>
            <Upload className={`w-8 h-8 ${isDragActive ? 'text-primary-600' : 'text-slate-400'}`} />
          </div>
          ...
          <p className="text-xs text-slate-400">
            Supported: JPEG, PNG, WebP, GIF, PDF, DOC (Max {maxFiles} files, 10MB each)
          </p>
        </div>
      </div>
      ...
      {uploading && (
        <div className="mt-4 p-4 bg-slate-50 rounded-xl">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-slate-700">Uploading {completedCount}/{files.length} files...</span>
            ...
            {files.map((fileObj) => (
              <div 
                ...
                className={`
                  ...
                  ${fileObj.status === 'completed' ? 'border-green-500' : 
                    fileObj.status === 'error' ? 'border-red-500' : 'border-slate-200'}
                `}
              >
                {/* Preview */}
                {fileObj.preview ? (
                ...
                <div className="p-2 bg-white">
                  <p className="text-xs font-medium text-slate-900 truncate" title={fileObj.name}>
                    {fileObj.name}
                  </p>
